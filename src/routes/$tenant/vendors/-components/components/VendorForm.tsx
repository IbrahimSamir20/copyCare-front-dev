...
import { VendorSchema, baseSchema } from '../schema/vendor.schema';
import { zodResolver } from '@hookform/resolvers/zod';
import { Button, Form, Input, InputNumber, Select } from 'antd';
import toast from 'react-hot-toast';
import { useCreateVendorMutation, useUpdateVendorMutation } from '../queries/vendor.query';
...
const VendorForm: FC<Props> = ({ id, data }) => {
  ...
  const { mutateAsync: addVendor } = useCreateVendorMutation();
  const { mutateAsync: editVendor } = useUpdateVendorMutation();
  const onsubmit = async (data: VendorSchema) => {
    await toast.promise(id ? editVendor({ ...(data as any), id }) : addVendor(data), {
      loading: `${id ? 'updating' : 'creating'} vendor`,
      success: `vendor ${id ? 'updated' : 'created'}`,
      error: `failed to ${id ? 'update' : 'create'} vendor`,
    });
    navigate({ to: '/app/vendors', search: { limit: 10, page: 1 } });
  };

  return (
    <FormProvider {...methods}>
      <Form layout="vertical" onFinish={handleSubmit(onsubmit)} className="flex flex-col gap-3">
        <ExpandPanel panelId={1} title="Vendor Info" activePanel={activePanel} setActivePanel={setActivePanel} required>
          <Form.Item
        ...
        <div className="col-span-2 flex w-full gap-3">
          ...
          <Link to="/app/vendors" search={{ limit: 10, page: 1 }}>
